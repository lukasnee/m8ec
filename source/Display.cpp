...
bool Display::init() {
    if (this->ili9341) {
        LOG("error: Display::init: already initialized\n");
        return false;
    }
    this->ili9341 = ili9341_new(
        ...
        [](uint32_t delay_ms) { vTaskDelay(pdMS_TO_TICKS(delay_ms)); }, TFT_RESET_GPIO_Port, TFT_RESET_Pin, TFT_CS_GPIO_Port,
        TFT_CS_Pin, TFT_DC_GPIO_Port, TFT_DC_Pin, isoLandscape, NULL, 0, NULL, 0, itsNONE, itnNONE);
    if (!this->ili9341) {
