...
bool EventDrivenReaderWriter::read(std::uint8_t *data, std::size_t size, TickType_t timeout_ticks) {
...
void EventDrivenReaderWriter::ll_async_read_completed_cb_from_isr() {
    BaseType_t *pxHigherPriorityTaskWoken = nullptr;
    this->read_semaphore.GiveFromISR(pxHigherPriorityTaskWoken);
}

void EventDrivenReaderWriter::ll_async_read_completed_cb() { this->read_semaphore.Give(); }
